-- Concurrency and write-performance tuning for attendance scans and high-volume imports

create index if not exists idx_attendance_events_ref_date_event_scanned
  on public.attendance_events (beneficiary_ref, attendance_date, event_type, scanned_at desc);

create index if not exists idx_attendance_events_date_status
  on public.attendance_events (attendance_date, status);

create index if not exists idx_beneficiaries_lookup_name_barangay
  on public.beneficiaries (barangay_id, last_name, first_name, middle_name);

create index if not exists idx_beneficiaries_batch_archived
  on public.beneficiaries (program_batch, is_archived);
...
begin
  ...
  if v_status = 'accepted' and v_event_type is not null then
    perform pg_advisory_xact_lock(
      hashtext(v_beneficiary.id::text),
      hashtext(v_attendance_date::text || ':' || v_event_type)
    );
