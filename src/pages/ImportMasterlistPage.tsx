...
export const ImportMasterlistPage = () => {
  ...
  return (
    <div className="space-y-4">
      <Card title="Import Masterlist" className="space-y-3">
        <div className="space-y-2 border-2 border-black bg-white p-3">
          ...
          <input
            type="file"
            accept=".csv,.xlsx"
            className="block w-full border-2 border-black bg-white p-2 text-sm"
            onChange={(event) => {
        ...
        {importFailures.length > 0 ? (
          <div className="border-2 border-black bg-white p-2 text-xs">
      ...
      <Modal open={confirmOpen} title="Confirm Import" onClose={() => setConfirmOpen(false)}>
        <div className="space-y-3 text-sm">
          <p>
            File: <span className="font-mono">{file?.name ?? 'N/A'}</span>
          </p>
          <p>Valid rows to import: {rows.length}</p>
          <p>Invalid rows (will be skipped): {invalidRows.length}</p>
          <div className="flex gap-2">
            <Button onClick={() => void runImport()} disabled={isImporting || rows.length === 0}>
